document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const rates = JSON.parse(localStorage.getItem('rates')) || { water: 18, electric: 8, service: 500, parking: 300 };
    ...
    function renderBillings() {
      ...
      billings.forEach((bill, index) => {
        ...
        const total = waterCost + electricCost + rates.service + rates.parking;
        const row = document.createElement('tr');
        row.innerHTML = `
          <td>${bill.room}</td>
          <td>${waterCost} บาท</td>
